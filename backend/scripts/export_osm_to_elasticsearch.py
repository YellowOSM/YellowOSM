...
parser.add_argument(
    "--split",
    action="store_true",
    help="split json output file into multiple files;"
    "this helps with elasticsearch import",
)
parser.add_argument(
    "--no-query",
    action="store_false",
    help="don't query db, just reformat existing dump.csv",
)
...
if SERVER:
    ...
    query_prefix = "docker exec $(docker ps | grep yosm_postgres |"
    " awk '{{print $1}}') psql -U postgres -d gis -c \"\copy ("
else:
    query_prefix = 'sudo -u postgres psql -d gis -c "\copy ('

# # COMMAND = "sudo -u postgres psql -d gis -c \"SELECT P.name,N.lon,N.lat,P.{key} from
# planet_osm_nodes N, planet_osm_point P WHERE N.id = P.osm_id AND P.{key} = '{value}'
# LIMIT {limit};\" | cat >> {file}"
# # COMMAND = "sudo -u postgres psql -d gis -c \"\copy (SELECT P.name,N.lon,N.lat,P.{key}
# from planet_osm_nodes N, planet_osm_point P WHERE N.id = P.osm_id AND P.{key} =
# '{value}' LIMIT {limit}) TO STDOUT With CSV;\" | cat >> {file}"
# COMMAND = "sudo -u postgres psql -d gis -c \"\copy (SELECT
# p.name,l.name,lon,lat,p.{key},l.{key}
# FROM planet_osm_nodes AS n LEFT JOIN planet_osm_point AS p ON n.id = p.osm_id LEFT
# JOIN planet_osm_polygon AS l ON n.id = l.osm_id WHERE p.{key} = '{value}' OR
# l.{key} = '{value}' LIMIT {limit}) TO STDOUT With CSV;\" | cat >> {file}"

COMMAND1 = (
    query_prefix
    + """
        SELECT p.name,st_x(st_transform(p.way, 4326)),st_y(st_transform(p.way, 4326)),
        p.{key},'n',
        *
...
if poly:
    COMMAND2 = (
        query_prefix
        + """
        SELECT p.name,st_x(st_transform(st_centroid(p.way), 4326)),
        st_y(st_transform(st_centroid(p.way), 4326)),p.{key},'w',
        *
...
export_emergency = {"key": "emergency", "values": ["defibrillator"]}
export_place = {"key": "place", "values": ["city", "town", "village"]}
export_industrial = {
    "key": "industrial",
    "values": [
        "aluminium_smelting",
...
if query_db:
        ...
        for command in commands[:2]:
            if command and "{value}" in command:
                for val in cl["values"]:
                    command_now = command.format(
                        key=cl["key"], value=val, file=EXPORT_FILE, limit=LIMIT
                    )
                    print(command_now)
    ...
    for cl in any_classes:
        for command in commands[2:4]:
            if command and "{value}" not in command:
                command_now = command.format(
                    key=cl["key"], file=EXPORT_FILE, limit=LIMIT
                )
                print(command_now)
                os.system(command_now)
